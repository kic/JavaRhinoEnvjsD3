...
public class LauncherImpl {
    ...
    public static void launchApplication(
            final Class<? extends Application> appClass, final String[] args) {
    ...
    public static void launchApplication(
        ...
        if (preloaderClass != null
                && !Preloader.class.isAssignableFrom(preloaderClass)) {
            throw new IllegalArgumentException("Error: "
                    + preloaderClass.getName()
        ...
        Thread launcherThread = new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    launchException = new RuntimeException(
                            "Application launch exception", ex);
                } catch (Error err) {
                    launchException = new RuntimeException(
                            "Application launch error", err);
                } finally {
    ...
    public static void launchApplication(final String launchName,
            final String launchMode, final String[] args) {
        /*
        ...
        if (launchMode.equals(LAUNCH_MODE_JAR)) {
            ...
            if (fxClassPath != null) {
                if (fxClassPath.trim().length() == 0) {
                    ...
                    if (verbose) {
                        System.err
                                .println("WARNING: Application jar uses deprecated JavaFX-Class-Path attribute."
                     ...
                     * anything
                     */
                    appLoader = setupJavaFXClassLoader(
                            new File(launchName), fxClassPath);
                }
            }

            // Support JavaFX-Feature-Proxy (only supported setting is 'auto', anything else is ignored)
            String proxySetting = jarAttrs
                    .getValue(MF_JAVAFX_FEATURE_PROXY);
            if (proxySetting != null
                    && "auto".equals(proxySetting.toLowerCase())) {
                trySetAutoProxy();
            }
            ...
            if (mainClassName == null) {
                ...
                if (mainClassName == null) {
                    ...
                    abort(null,
                            "JavaFX jar manifest requires a valid JavaFX-Appliation-Class or Main-Class entry");
                }
            }
            ...
            abort(new IllegalArgumentException(
                    "The launchMode argument must be one of LM_CLASS or LM_JAR"),
        ...
        if (appLoader != null) {
            try {
                ...
                Class<?> launcherClass = appLoader
                        .loadClass(LauncherImpl.class.getName());

                // then invoke the second part of this launcher using reflection
                Method lawa = launcherClass.getMethod(
                ...
                lawa.invoke(null, new Object[] { mainClassName,
                        preloaderClassName, appArgs });
            } catch (Exception e) {
                abort(e, "Exception while launching application");
            }
        } else {
            launchApplicationWithArgs(mainClassName, preloaderClassName,
                    appArgs);
        }
    }
    ...
    public static void launchApplicationWithArgs(String mainClassName,
        ...
        try {
            ...
            abort(cnfe, "Missing JavaFX application class %1$s",
                    mainClassName);
        }
        ...
        if (!Application.class.isAssignableFrom(tempClass)) {
            ...
            try {
                ...
                abort(null,
                        "JavaFX application class %1$s does not extend javafx.application.Application",
                        tempClass.getName());
                ...
                ex.printStackTrace();
                abort(null, "Exception running application %1$s",
                        tempClass.getName());
            }
            return;
        }
        ...
        if (preloaderClassName != null) {
            try {
                ...
                abort(cnfe, "Missing JavaFX preloader class %1$s",
                        preloaderClassName);
            }

            if (!Preloader.class.isAssignableFrom(tempClass)) {
                abort(null,
                        "JavaFX preloader class %1$s does not extend javafx.application.Preloader",
                        preClass.getName());
            }
    ...
    private static ClassLoader setupJavaFXClassLoader(File appJar,
            String fxClassPath) {
        try {
            ...
            if (cp != null) {
                ...
                while (cp.length() > 0) {
                    ...
                    if (pathSepIdx < 0) {
                        String pathElem = cp;
                        File f = (baseDir == null) ? new File(pathElem)
                                : new File(baseDir, pathElem);
                        if (f.exists()) {
                            jcpList.add(fileToURL(f));
                        } else if (verbose) {
                            System.err.println("Class Path entry \""
                                    + pathElem
            ...
            if (!jcpList.isEmpty()) {
                ...
                URL jfxRtURL = LauncherImpl.class.getProtectionDomain()
            ...
            if (trace) {
                System.err
                        .println("Exception creating JavaFX class loader: "
                                + ex);
                ex.printStackTrace();
            }
        }
    ...
    private static void trySetAutoProxy() {
        ...
        if (System.getProperty("http.proxyHost") != null
            ...
            if (verbose) {
                System.out
                        .println("Explicit proxy settings detected. Skip autoconfig.");
                System.out.println("  http.proxyHost="
                        + System.getProperty("http.proxyHost"));
                System.out.println("  https.proxyHost="
                        + System.getProperty("https.proxyHost"));
                System.out.println("  ftp.proxyHost="
        ...
        try {
            ...
            if (trace) {
                System.err.println("Unable to build URL to deploy.jar: "
                        + ex);
                ex.printStackTrace();
            }
        ...
        try {
            ...
            Class sm = Class.forName(
                    "com.sun.deploy.services.ServiceManager", true, dcl);
            ...
            Class pt = Class.forName(
                    "com.sun.deploy.services.PlatformType", true, dcl);
            values[0] = pt.getField(servicename).get(null);
            setservice.invoke(null, values);

            Class dps = Class.forName(
                    "com.sun.deploy.net.proxy.DeployProxySelector", true,
                    dcl);
            ...
            if (verbose) {
                System.err
                        .println("Failed to autoconfig proxy due to " + e);
            }
            if (trace) {
                e.printStackTrace();
            }
    ...
    private static String[] getAppArguments(Attributes attrs) {
        ...
        try {
            ...
            while (attrs.getValue(paramNamePrefix + idx) != null) {
                String k = decodeBase64(attrs.getValue(paramNamePrefix
                        + idx));
                String v = null;
            ...
            if (verbose) {
                System.err
                        .println("Failed to extract application parameters");
            }
            ioe.printStackTrace();
        }
    ...
    private static void abort(final Throwable cause, final String fmt,
            final Object... args) {
    ...
    private static void launchApplication1(
            final Class<? extends Application> appClass,
            final Class<? extends Preloader> preloaderClass,
        ...
        try {
            ...
            if (preloaderClass != null) {
                ...
                try {
                    Constructor<? extends Preloader> c = preloaderClass
                            .getConstructor();
                    pldr = c.newInstance();
                    // Set startup parameters
                    ParametersImpl.registerParameters(pldr,
                            new ParametersImpl(args));
                } catch (Throwable t) {
                    System.err
                            .println("Exception in Preloader constructor");
                    pConstructorError = t;
                    error = true;
                }
            }
            ...
            if (currentPreloader != null && !error && !exitCalled.get()) {
                try {
                    ...
                    System.err
                            .println("Exception in Preloader init method");
                    pInitError = t;
                    error = true;
                }
            }
                ...
                PlatformImpl.runAndWait(new Runnable() {
                    ...
                    public void run() {
                        try {
                            ...
                            application = currentPreloader;
                            currentPreloader.start(primaryStage);
                        } catch (Throwable t) {
                            System.err
                                    .println("Exception in Preloader start method");
                            pStartError = t;
                            error = true;
                        }
                    }
            ...
            if (!error && !exitCalled.get()) {
                if (currentPreloader != null) {
                    if (simulateSlowProgress) {
                        for (int i = 0; i < 100; i++) {
                            notifyProgress(currentPreloader,
                                    (double) i / 100.0);
                            Thread.sleep(10);
                        }
                    }
                ...
                try {
                    Constructor<? extends Application> c = appClass
                            .getConstructor();
                    app = c.newInstance();
                    System.out.println("test " + app);
                    application = app;
                    // Set startup parameters
                    ParametersImpl.registerParameters(app,
                            new ParametersImpl(args));
                } catch (Throwable t) {
                    System.err
                            .println("Exception in Application constructor");
                    constructorError = t;
                    error = true;
                }
            }
            ...
            if (!error && !exitCalled.get()) {
                ...
                try {
                    ...
                    System.err
                            .println("Exception in Application init method");
                    initError = t;
                    error = true;
                }
            }
                ...
                PlatformImpl.runAndWait(new Runnable() {
                    ...
                    public void run() {
                        try {
                            ...
                            System.err
                                    .println("Exception in Application start method");
                            startError = t;
                            error = true;
                        }
                    }
                ...
                PlatformImpl.runAndWait(new Runnable() {
                    ...
                    public void run() {
                        try {
                            ...
                            System.err
                                    .println("Exception in Application stop method");
                            stopError = t;
                            error = true;
                        }
                    }
            ...
            if (error) {
                if (pConstructorError != null) {
                    throw new RuntimeException(
                            "Unable to construct Preloader instance: "
                                    + appClass, pConstructorError);
                } else if (pInitError != null) {
                    throw new RuntimeException(
                            "Exception in Preloader init method",
                            pInitError);
                    ...
                    throw new RuntimeException(
                            "Exception in Preloader stop method",
                            pStopError);
                } else if (constructorError != null) {
                    String msg = "Unable to construct Application instance: "
                            + appClass;
            ...
            if (error && isJavaws) {
                System.err
                        .println("Workaround until RT-13281 is implemented: keep toolkit alive");
            } else {
                PlatformImpl.tkExit();
            }
        }
    ...
    private static void notifyStateChange(final Preloader preloader,
            final StateChangeNotification.Type type, final Application app) {

        PlatformImpl.runAndWait(new Runnable() {
            @Override
            public void run() {
                preloader
                        .handleStateChangeNotification(new Preloader.StateChangeNotification(
                                type, app));
            }
        });
    }

    private static void notifyProgress(final Preloader preloader,
            final double d) {
        PlatformImpl.runAndWait(new Runnable() {
            @Override
            public void run() {
                preloader
                        .handleProgressNotification(new Preloader.ProgressNotification(
                                d));
            }
        });
    }

    private static boolean notifyError(final String msg,
            final Throwable constructorError) {
        ...
        PlatformImpl.runAndWait(new Runnable() {
            ...
            public void run() {
                if (currentPreloader != null) {
                    try {
                        ErrorNotification evt = new ErrorNotification(null,
                                msg, constructorError);
                        boolean rval = currentPreloader
                                .handleErrorNotification(evt);
                        result.set(rval);
                    } catch (Throwable t) {
                        t.printStackTrace();
                    }
    ...
    private static void notifyCurrentPreloader(
            final PreloaderNotification pe) {
        PlatformImpl.runAndWait(new Runnable() {
            @Override
            public void run() {
                if (currentPreloader != null) {
    ...
    public static void notifyPreloader(Application app,
            final PreloaderNotification info) {
        if (launchCalled.get()) {
        ...
        synchronized (LauncherImpl.class) {
            if (notifyMethod == null) {
                final String fxPreloaderClassName = "com.sun.deploy.uitoolkit.impl.fx.FXPreloader";
                try {
                    Class fxPreloaderClass = Class
                            .forName(fxPreloaderClassName);
                    notifyMethod = fxPreloaderClass.getMethod(
                            "notifyCurrentPreloader",
                            PreloaderNotification.class);
                } catch (Exception ex) {
                    ex.printStackTrace();
                    return;
                }
